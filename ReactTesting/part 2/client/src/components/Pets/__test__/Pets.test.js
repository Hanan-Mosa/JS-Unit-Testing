...
describe("Test Pets component", () => {
  ...
  test("Test render after Gender Filter", async () => {
   ...
   let catCards= await screen.findAllByRole("article");
    ...
    expect(screen.getAllByRole("article").length).toBe(2);
    expect(screen.getAllByRole('article')).toStrictEqual([catCards[0],catCards[1]])
  });

   test("Test render after nofavourite Filter", async () => {
    render(<Pets />);
   let catCards= await screen.findAllByRole("article");
    fireEvent.change(screen.getByLabelText(/Favourite/i), { target: { value: "not favoured" } });
    expect(screen.getAllByRole("article").length).toBe(3);
    expect(screen.getAllByRole('article')).toStrictEqual([catCards[2],catCards[3],catCards[4]])
  });
});
